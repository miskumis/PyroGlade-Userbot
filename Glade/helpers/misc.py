...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("Glade").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("Glade").info(f"Invalid Git Command")
        ...
        LOGGER("Glade").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("Glade").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk PyroGlade-UserBot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @Lunatic0de ✨"
    try:
