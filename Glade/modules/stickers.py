...
from Glade.helpers.tools import get_arg, get_text, resize_media
from Glade.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    Glade = await edit_or_reply(message, "`Boleh juga ni stickernya colong ahh...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Glade.edit("**Stiker tidak memiliki Nama!**")
                return
            ...
            await Glade.edit("**File Tidak Didukung**")
            return
        media_ = await client.download_media(replied, file_name="Glade/resources/")
    else:
        await Glade.edit("**Silahkan Reply ke Media Foto/GIF/Sticker!**")
        return
    if media_:
        ...
        if exist is not False:
            try:
                ...
                return await Glade.edit(f"**ERROR:** `{e}`")
            ...
            await Glade.edit("`Membuat Sticker Pack Baru`")
            try:
                await client.send_message("Stickers", cmd)
            except YouBlockedUser:
                await client.unblock_user("stickers")
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("Glade/resources/blank.png")
    if ik.endswith(".tgs"):
        await client.download_media(reply, "Glade.tgs")
        await bash("lottie_convert.py Glade.tgs json.json")
        json = open("json.json", "r")
        jsn = json.read()
        jsn = jsn.replace("512", "2000")
        ("json.json", "w").write(jsn)
        await bash("lottie_convert.py json.json Glade.tgs")
        file = "Glade.tgs"
        os.remove("json.json")
