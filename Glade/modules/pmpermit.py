...
from Glade import TEMP_SETTINGS
from Glade.helpers.adminHelpers import DEVS
from Glade.helpers.basic import edit_or_reply
from Glade.helpers.SQL.globals import addgvar, gvarstatus
from Glade.helpers.tools import get_arg
...
async def incomingpm(client: Client, message: Message):
    try:
        from Glade.helpers.SQL.globals import gvarstatus
        from Glade.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from Glade.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>Terdeteksi Developer PyroGlade-Userbot</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from Glade.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from Glade.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from Glade.helpers.SQL.globals import addgvar
    ...
    Glade = await cust_msg.edit("`Processing...`")
    if input_str and not input_str.isnumeric():
        return await Glade.edit("**Harap masukan angka untuk PM_LIMIT.**")
    addgvar("PM_LIMIT", input_str)
    await Glade.edit(f"**Set PM limit to** `{input_str}`")
...
async def setpmpermit(client: Client, cust_msg: Message):
    ...
    try:
        import Glade.helpers.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    Glade = await cust_msg.edit("`Processing...`")
    ...
    if not message:
        return await Glade.edit("**Mohon Reply Ke Pesan**")
    msg = message.text
    sql.addgvar("unapproved_msg", msg)
    await Glade.edit("**Pesan Berhasil Disimpan Ke Room Chat**")
...
async def get_pmermit(client: Client, cust_msg: Message):
    ...
    custom_message = sql.gvarstatus("unapproved_msg")
    if custom_message is not None:
        await Glade.edit("**Pesan PMPERMIT Yang Sekarang:**" f"\n\n{custom_message}")
    else:
        await Glade.edit(
...
async def reset_pmpermit(client: Client, cust_msg: Message):
    ...
    if custom_message is None:
        await Glade.edit("**Pesan PMPERMIT Anda Sudah Default**")
    else:
        sql.delgvar("unapproved_msg")
        await Glade.edit("**Berhasil Mengubah Pesan Custom PMPERMIT menjadi Default**")
