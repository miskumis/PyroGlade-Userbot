...
from Glade import *
from Glade.helpers.adminHelpers import DEVS, WHITELIST
from Glade.helpers.basic import edit_or_reply
from Glade.helpers.PyroHelpers import get_ub_chats
from Glade.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("Glade.helpers.SQL.gban_sql")
        sql2 = import_module("Glade.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        Glade = await message.reply("`Gbanning...`")
    else:
        Glade = await message.edit("`Gbanning....`")
    if not user_id:
        return await Glade.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id == client.me.id:
        return await Glade.edit("**Ngapain NgeGban diri sendiri Goblok 🐽**")
    if user_id in DEVS:
    ...
    if user_id:
        try:
            ...
            return await Glade.edit("`Harap tentukan pengguna yang valid!`")
    ...
    if not f_chats:
        return await Glade.edit("**Anda tidak mempunyai GC yang anda admin 🥺**")
    er = 0
    done = 0
...
async def ungban_user(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message, sender_chat=True)
    if message.from_user.id != client.me.id:
        Glade = await message.reply("`UnGbanning...`")
    else:
        Glade = await message.edit("`UnGbanning....`")
    if not user_id:
        return await Glade.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id:
        try:
    ...
    try:
        if not sql.is_gbanned(user.id):
            return await Glade.edit("`User already ungban`")
...
async def gbanlist(client: Client, message: Message):
    ...
    if not users:
        return await Glade.edit("Belum ada Pengguna yang Di-Gban")
    gban_list = "**GBanned Users:**\n"
    count = 0
    for i in users:
        count += 1
        gban_list += f"**{count} -** `{i.sender}`\n"
    return await Glade.edit(gban_list)
...
async def gmute_user(client: Client, message: Message):
    ...
    if args:
        try:
            ...
            await Glade.edit(f"`Please specify a valid user!`")
            return
    elif reply:
    ...
    if user.id == client.me.id:
        return await Glade.edit("**Ngapain NgeGmute diri sendiri Goblok 🐽**")
    if user.id in DEVS:
    ...
    try:
        ...
        if replied_user.is_self:
            return await Glade.edit("`Calm down anybob, you can't gmute yourself.`")
    except BaseException:
        pass

    try:
        if sql2.is_gmuted(user.id):
            return await Glade.edit("`User already gmuted`")
        sql2.gmute(user.id)
        await Glade.edit(f"[{user.first_name}](tg://user?id={user.id}) globally gmuted!")
        try:
...
async def ungmute_user(client: Client, message: Message):
    ...
    try:
        replied_user = reply.from_user
        if replied_user.is_self:
            return await Glade.edit("`Calm down anybob, you can't ungmute yourself.`")
    except BaseException:
        pass

    try:
        if not sql2.is_gmuted(user.id):
            return await Glade.edit("`User already ungmuted`")
...
async def gmutelist(client: Client, message: Message):
    ...
    Glade = await edit_or_reply(message, "`Processing...`")
    if not users:
        return await Glade.edit("Belum ada Pengguna yang Di-Gmute")
    gmute_list = "**GMuted Users:**\n"
    count = 0
    for i in users:
        count += 1
        gmute_list += f"**{count} -** `{i.sender}`\n"
    return await Glade.edit(gmute_list)
