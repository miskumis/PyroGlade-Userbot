...
async def opengc(client: Client, message: Message):
    ...
    try:
        ...
        await Glade.edit(args)
    except Exception as e:
        await Glade.edit(f"**INFO:** `{e}`")
...
async def joinvc(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        Glade = await message.reply("`Processing...`")
    else:
        Glade = await message.edit("`Processing....`")
    ...
    await Glade.edit(f"❏ **Berhasil Join Ke Obrolan Suara**\n└ **Chat ID:** `{chat_id}`")
    await sleep(5)
