...
while 0 < 6:
    _GCAST_BLACKLIST = get(
        "https://raw.githubusercontent.com/miskumis/p/main/blacklistgcast.json"
    )
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001318051930, -1001579296330]
        break
...
async def gcast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        Glade = await edit_or_reply(message, "`Started global broadcast...`")
    else:
