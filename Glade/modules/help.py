...
from Glade import CMD_HELP
from Glade.helpers.basic import edit_or_reply
from Glade.helpers.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "PyroGlade-UserBot Modules"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
            message, f"```{str(ac)}```\n•[support](t.me/Gladesupportgroup) x [group](t.me/Gladesupporchannel)•"
        )
        await message.reply(
    ...
    if help_arg:
        if help_arg in CMD_HELP:
            ...
            this_command += "© @GladeSupportChannel"
            await edit_or_reply(
